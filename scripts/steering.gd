...
func flee(target):
	var desired_vec = get_global_transform().origin - target.get_global_transform().origin
...
func pursuit(evader):
	...
	var relative_heading = get_rotation().dot(evader.get_rotation())
	if(curr_pos.distance_to(evader_pos) > 0 and relative_heading < -0.95):
		...
		return seek_pos(evader_pos + evader.get_linear_velocity() * look_ahead_time)
